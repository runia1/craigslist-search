...
async function getCityLinks() {
  ...
  const cityLinksForStates = await Promise.all(
    stateLinks.map(async (stateLink) => {
      const page = await browser.newPage();
      await page.goto(`https:${stateLink}`);
      const stateCityLinks = await page.evaluate(() =>
        Array.from(
          document.querySelectorAll(
            'ul[class*="geo-site-list"]>li>a[href*="craigslist.org"]'
          ),
          (a) => a.getAttribute("href")
        )
      );
  ...
  try {
    ...
    try {
      ...
      return await page.evaluate(() =>
        Array.from(document.querySelectorAll('li[class="result-row"]>a'), (a) =>
          a.getAttribute("href")
        )
      );
    } catch (error) {
...
(async () => {
  ...
  let postsOfInterest = [];
  await Promise.all(
    queue.map(async (cityLink) => {
      try {
        const page = await browser.newPage();

        do {
          const foundResults = await getResultsFromCity(
            page,
            cityLink,
            category,
            searchQuery
...
// https://phoenix.craigslist.org/wvl/avo/d/glendale-stol-bush-plane-folding-wing/7445987761.html
// https://sandiego.craigslist.org/nsd/avo/d/ramona-zenith-ch750-stol-aircraft/7463035590.html
// https://lewiston.craigslist.org/avo/d/grangeville-kitfox-avid-main-gear/7444677265.html
